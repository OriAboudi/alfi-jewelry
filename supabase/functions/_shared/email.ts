...
// RESEND_API_KEY is a Supabase secret — never hardcoded. If it isn't set yet
// (e.g. before the account is created), this quietly no-ops so orders still
// go through; it just logs a warning instead of emailing.
...
export async function sendOrderConfirmationEmail(order: any) {
  ...
  if (!trackUrl) console.warn("SITE_URL not set — confirmation email will omit the order-tracking link");

  const html = `
    <div dir="rtl" style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#3a2c25;">
      <h2 style="color:#bd7355;">תודה על ההזמנה!</h2>
      <p>ההזמנה שלך <strong>${escapeHtml(order.number || "")}</strong> התקבלה בהצלחה.</p>
      ...
      ${trackUrl ? `<p style="margin-top:24px;"><a href="${trackUrl}" style="display:inline-block;background:#bd7355;color:#fff;text-decoration:none;padding:12px 24px;border-radius:10px;font-weight:bold;">מעקב אחר ההזמנה</a></p>
