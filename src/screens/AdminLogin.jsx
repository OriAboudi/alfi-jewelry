    ...
    <div style={css("min-height:100vh;display:flex;align-items:center;justify-content:center;background:#faf5ef;")}>
      <div style={css("width:100%;max-width:360px;padding:40px;background:#fff;border-radius:18px;box-shadow:0 24px 70px rgba(70,50,40,.12);")}>
        ...
        <div style={css("margin-bottom:16px;")}>
          <label style={css("display:block;font-size:13px;color:#8a766a;margin-bottom:7px;")}>אימייל</label>
          <input
            value={adminForm.email}
            onChange={(e) => setAdminField("email", e.target.value)}
            style={css("width:100%;padding:13px 15px;border:1px solid #e7d8cb;border-radius:12px;font-size:15px;background:#fff;")}
          />
        </div>
        <div style={css("margin-bottom:18px;")}>
          <label style={css("display:block;font-size:13px;color:#8a766a;margin-bottom:7px;")}>סיסמה</label>
          <input
            type="password"
        ...
        {adminError && (
          <div style={css("background:#fbeae4;color:#a85a44;font-size:13.5px;padding:11px 14px;border-radius:10px;margin-bottom:16px;")}>
            {adminError}
          </div>
        )}
        <button
          onClick={submitAdminLogin}
          style={css(`width:100%;padding:15px;background:#bd7355;color:#fff;border:none;border-radius:12px;font-size:15.5px;font-weight:600;cursor:pointer;opacity:${adminBusy ? 0.6 : 1};`)}
        >
          {adminBusy ? "רגע..." : "כניסה"}
        </button>
      </div>
