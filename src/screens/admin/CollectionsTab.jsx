    ...
    <div>
      ...
      {collections.map((c) => (
        <div key={c.id} className="r-admin-row" style={css("display:flex;align-items:center;gap:18px;background:#fff;border:1px solid #ecdccd;border-radius:14px;padding:14px 18px;margin-bottom:10px;")}>
          ...
          <div style={css("flex:1;")}><div style={css("font-family:'Frank Ruhl Libre',serif;font-size:17px;")}>{c.title}</div><div style={css("font-size:13px;color:#8a766a;")}>{c.subtitle}{c.category_filter ? ` · מסונן: ${c.category_filter}` : ""}</div></div>
