...
export function PaymentTestsTab() {
  ...
  return (
    <div>
      <h2 style={css("font-family:var(--font-serif);font-weight:400;font-size:24px;margin-bottom:12px;")}>בדיקות תשלום</h2>
      {BACKEND !== "supabase" ? (
        <div style={css("background:#fff;border:1px dashed var(--c-line-strong);border-radius:14px;padding:40px;text-align:center;color:var(--c-ink-mute);")}>
          זמין רק כשה‑backend הוא Supabase (יש חיבור לשערי תשלום אמיתיים).
        </div>
      ) : (
        <div style={css("background:#fff;border:1px solid var(--c-line);border-radius:14px;padding:24px;max-width:420px;")}>
          <p style={css("font-size:13.5px;color:var(--c-ink-mute);margin-bottom:18px;line-height:1.6;")}>
