...
export const lbl = "display:block;font-size:13px;font-weight:600;color:#8a766a;margin-bottom:7px;";
export const inp = "width:100%;padding:12px 14px;border:1px solid #e7d8cb;border-radius:11px;font-size:15px;background:#fff;";
...
export function OverlayHeader({ title, onClose }) {
  return (
    <div style={css("display:flex;justify-content:space-between;align-items:center;margin-bottom:22px;")}>
      <h2 style={css("font-family:'Frank Ruhl Libre',serif;font-weight:400;font-size:26px;")}>{title}</h2>
      <span onClick={onClose} style={css("cursor:pointer;font-size:24px;color:#8a766a;")}>×</span>
    </div>
  );
}
...
export function Pager({ page, pageSize, count, onPage }) {
  ...
  return (
    <div style={css("display:flex;justify-content:center;align-items:center;gap:14px;margin-top:22px;")}>
      <button onClick={() => onPage(Math.max(1, page - 1))} disabled={page <= 1} style={css(`padding:8px 16px;border:1px solid #e0cdbd;border-radius:9px;background:#fff;cursor:pointer;font-size:13.5px;opacity:${page <= 1 ? 0.5 : 1};`)}>הקודם</button>
      <span style={css("font-size:13.5px;color:#8a766a;")}>עמוד {page} מתוך {pages}</span>
      <button onClick={() => onPage(Math.min(pages, page + 1))} disabled={page >= pages} style={css(`padding:8px 16px;border:1px solid #e0cdbd;border-radius:9px;background:#fff;cursor:pointer;font-size:13.5px;opacity:${page >= pages ? 0.5 : 1};`)}>הבא</button>
    </div>
  );
}
