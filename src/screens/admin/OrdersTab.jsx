...
export function OrdersTab() {
  ...
  return (
    <div>
      <h2 style={css("font-family:'Frank Ruhl Libre',serif;font-weight:400;font-size:24px;margin-bottom:18px;")}>הזמנות ({count})</h2>
      ...
      {loading ? (
        ...
        <div style={css("background:#fff;border:1px dashed #e0cdbd;border-radius:14px;padding:40px;text-align:center;color:#8a766a;")}>לא נמצאו הזמנות.</div>
      ) : rows.map((o) => {
        ...
        return (
          <div key={o.id} className="r-admin-row" style={css(`background:#fff;border:1px solid ${o.is_test ? "#e0cdbd" : "#ecdccd"};border-radius:14px;padding:18px 20px;margin-bottom:10px;display:flex;align-items:center;gap:18px;flex-wrap:wrap;${o.is_test ? "opacity:.7;" : ""}`)}>
            ...
            <div style={css("flex:1;min-width:140px;")}><div style={css("font-weight:600;font-size:15.5px;")}>{o.number}</div><div style={css("font-size:13px;color:#8a766a;")}>{fmtDate(o.created_at)} · {cust}</div></div>
            ...
            <select value={o.status} onChange={(e) => changeStatus(o.id, e.target.value)} style={css("padding:8px 12px;border:1px solid #e7d8cb;border-radius:9px;font-size:13.5px;background:#fff;cursor:pointer;color:#3a2c25;")}>
        ...
        return (
          <Overlay onClose={() => setOrderView(null)}>
            ...
            <div style={css("display:flex;flex-direction:column;gap:18px;")}>
              ...
              {history.length > 0 && (
                <div style={css("background:#fff;border:1px solid #ecdccd;border-radius:14px;padding:18px;")}>
                  <div style={css("font-size:13px;font-weight:700;color:#8a766a;margin-bottom:10px;")}>ציר זמן סטטוס</div>
                  {history.map((h, i) => (
                    <div key={i} style={css("display:flex;justify-content:space-between;font-size:13.5px;padding:6px 0;border-top:1px solid #f3e8dd;")}>
                      <span>{h.status}</span>
                      <span style={css("color:#8a766a;")}>{fmtDateTime(h.changed_at)}{h.notified ? " · נשלח מייל" : ""}</span>
                    </div>
                  ))}
                </div>
              )}
